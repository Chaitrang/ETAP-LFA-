...
| **Short Circuit Study** | Short Circuit report | `Short Circuit Report.docx` + `.xlsx` |

Load Flow builds its table from the columns you select and is generated
dynamically — **no template**. Short Circuit fills its bundled template, which
the user never uploads. There are three front ends over one shared pipeline.
...
shortcircuit_parser.py  Short Circuit parser
shortcircuit_writer.py  Short Circuit writer (template-driven)
...
Unchanged: `ID kV % Mag. Ang. … Amp %PF %Tap` identifies the bus results,
`ID kV Rated Amp … MVA %PF Amp Loading` the Bus Loading Summary. Voltage % comes
from the first, kW (MVA × %PF), Amp Loading, Amp Rating and % Loading from the
second. Reactive and apparent loading come from the same two figures
(`Q = S·sin(acos(PF))`), so they are exactly consistent with the kW column.
...
## 4. Template-driven columns (Short Circuit)

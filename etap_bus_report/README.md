...
Qt window, so it works as the cloud main module even though the same file
launches the desktop app locally. (Streamlit Community Cloud does not let you
change an existing app's main file path, so this keeps a deployed app working
without deleting and redeploying it.)
...
fails with `libglib-2.0.so.0: cannot open shared object file`. PySide6 belongs
in `requirements-desktop.txt` only.
...
streamlit_app.py    web front end (render() is called once per Streamlit run)

...
def add_detail_fields(pdf_path: str, buses: list[dict]) -> list[dict]:
    """
    Add ``xr_ratio`` from ETAP's per-bus ``SHORT-CIRCUIT REPORT`` detail pages.

    Only called when the Word template maps a column that the summary table
    cannot supply, because it has to walk the whole document.
    """
    ...
    try:
        for page in pdf.pages:
            ...
            if not _DETAIL_BUS_RE.search(text):
                page.flush_cache()
                continue
            lines = [l.strip() for l in text.split("\n")]
            bus_id = None
            for index, line in enumerate(lines):
                if _DETAIL_BUS_RE.search(line) and index + 1 < len(lines):
                    bus_id = clean_bus_id(lines[index + 1])
                # "<ID> Total 0.00 0.132 -4.312 32.6 4.314" -> X/R is 2nd last
                if bus_id and " Total " in f" {line} ":
                    numbers = [to_float(t) for t in line.split()]
                    numbers = [n for n in numbers if n is not None]
                    if len(numbers) >= 2:
                        bus = wanted.get(bus_id)
                        if bus is not None and bus.get("xr_ratio") is None:

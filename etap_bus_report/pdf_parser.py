...
def _load_flow_columns(header: list[dict], groups: list[dict], page_width: float) -> ColumnMap:
    ...
    for word in tokens:
        ...
        if text == "id":
            ...
            mw_seen += 1
            # ETAP always prints the three MW columns in this order:
            # Generation, directly connected Load, branch Load Flow.
            # Position is authoritative; snapping to the nearest group label
            # above was unreliable and previously merged Generation with Load,
            # which left 'load_mw' undefined.
            anchors.append(
                ({1: "gen_mw", 2: "load_mw", 3: "flow_mw"}.get(mw_seen, f"mw_{mw_seen}"), pos)
            )
        elif text == "mvar":
            mvar_seen += 1
            anchors.append(
                (
                    {1: "gen_mvar", 2: "load_mvar", 3: "flow_mvar"}.get(
                        mvar_seen, f"mvar_{mvar_seen}"
                    ),
                    pos,
                )
            )
...
def _collect_flow(record: BusRecord, columns: ColumnMap, line: list[dict]) -> None:
    """Record one branch flow of a bus (MW/Mvar leaving or entering it)."""
    flow_mw = columns.value_in(line, "flow_mw")
    if flow_mw is None:
        return
    record.meta.setdefault("flows_mw", []).append(flow_mw)
    flow_mvar = columns.value_in(line, "flow_mvar")
    record.meta.setdefault("flows_mvar", []).append(
        flow_mvar if flow_mvar is not None else 0.0
    )
    ...
    for table in block:
        ...
        for line in table.lines[table.header_index + 1 :]:
            ...
            if bus_words and kv is not None and vmag is not None:
                ...
                load_mvar = columns.value_in(line, "load_mvar")
                if load_mvar is not None:
                    record.meta["load_mvar"] = load_mvar
                record.meta["flows_mw"] = []
                record.meta["flows_mvar"] = []
                _collect_flow(record, columns, line)
                ...
                # one of the bus's branch-flow lines
                _collect_flow(records[-1], columns, line)
...
def _total_bus_load(record: BusRecord) -> tuple[Optional[float], Optional[float]]:
    """
    Total bus load in MW/Mvar built from the Load Flow Report itself:
    the load connected directly to the bus plus everything flowing out of it.

    Both parts are printed to 3 decimals of MW, so this is accurate to about
    0.5 kW regardless of bus size.  ``None`` when the bus has no flow lines at
    all (nothing to add up).
    """
    flows_mw = record.meta.get("flows_mw")
    if flows_mw is None:
        return None, None
    flows_mvar = record.meta.get("flows_mvar") or []
    mw = (record.meta.get("load_mw") or 0.0) + sum(f for f in flows_mw if f > 0)
    mvar = (record.meta.get("load_mvar") or 0.0) + sum(
        q for f, q in zip(flows_mw, flows_mvar) if f > 0
    )
    return mw, mvar
...
def _merge(records: list[BusRecord], loading: dict[str, dict]) -> list[BusRecord]:
    ...
    kW / kvar are taken from the most precise source the report offers:

    1. **Load Flow Report flows** - directly connected load plus outgoing branch
       flows.  Printed to 3 decimals of MW, so accurate to ~0.5 kW at any bus
       size.  Used when it agrees with the Bus Loading Summary, which confirms
       the two describe the same quantity.
    2. **Bus Loading Summary** - total bus load MVA x %PF.  Correct, but ETAP
       prints %PF to one decimal, so the rounding error grows with bus size
       (~9 kW on a 56 MW bus).  Used when there are no flow lines to add up.
    3. **Directly connected load** alone, for buses the Loading Summary omits.
    """
    for record in records:
        data = loading.get(record.bus_id)
        flow_mw, flow_mvar = _total_bus_load(record)

        if data:
            mva, pf = data.get("mva"), data.get("pf")
            summary_kw = None
            if mva is not None and pf is not None:
                summary_kw = mva * (pf / 100.0) * 1000.0
            elif mva is not None:
                summary_kw = mva * 1000.0

            if summary_kw is not None:
                record.kw_loading = summary_kw
                record.meta["source_kw"] = "loading summary (MVA x %PF)"
                # Prefer the flows when they corroborate the summary: same
                # quantity, but without the %PF rounding error.
                if flow_mw is not None and abs(flow_mw * 1000.0 - summary_kw) <= max(
                    1.0, 0.01 * abs(summary_kw)
                ):
                    record.kw_loading = flow_mw * 1000.0
                    record.meta["source_kw"] = "load flow report (load + outgoing flows)"
                    if flow_mvar is not None:
                        record.meta["kvar_loading"] = flow_mvar * 1000.0
            ...
            record.meta["mva"] = mva
            record.meta["pf"] = pf
            if record.meta.get("kvar_loading") is None and mva is not None and pf is not None:
                # Q = S * sin(acos(PF)); consistent with P = S * PF above.
                ratio = max(0.0, 1.0 - (pf / 100.0) ** 2)
                record.meta["kvar_loading"] = mva * math.sqrt(ratio) * 1000.0

        if record.kw_loading is None and flow_mw is not None:
            record.kw_loading = flow_mw * 1000.0
            record.meta["source_kw"] = "load flow report (load + outgoing flows)"
            if record.meta.get("kvar_loading") is None and flow_mvar is not None:
                record.meta["kvar_loading"] = flow_mvar * 1000.0

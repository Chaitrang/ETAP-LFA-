...
# Public aliases of the low-level table helpers, so that other report writers
# (Short Circuit today, Motor Starting tomorrow) can reuse them without
# touching this module's Load Flow logic.
set_cell_text = _set_cell_text
append_row = _append_row
remove_row = _remove_row
resize_table = _resize_table
find_table = _bus_table


def build_document(buses: Sequence[BusRecord], template: Optional[str] = None) -> Document:
    """
    Return the populated :class:`docx.Document` without writing it to disk.

    Used by the desktop app (which then saves to a chosen path) and by the web
    app (which streams the bytes to the browser).
    """
...
def build_document_bytes(buses: Sequence[BusRecord], template: Optional[str] = None) -> bytes:
    """Populated document as raw ``.docx`` bytes (for downloads / in-memory use)."""
    import io

    buffer = io.BytesIO()
    build_document(buses, template).save(buffer)
...
def write_bus_report(
    ...
    """
    document = build_document(buses, template)
